defmodule ExApp.MnesiaTestTask do
  ...
  # for a row wich "id" column wich value equalt to "200". 
  # The "id" column is always the first table row defined, by convention.
  defp run_loop() do
    try do
      sleep_time = 10000
  
      # Machine A inserting data    
      ...
      # Data inserted by local node in machine A
      ...
      ["[user_id = 200] result node local =>", result] |> IO.inspect()
      ["[user_id = 300] result node remote => ", result_remote] |> IO.inspect()
